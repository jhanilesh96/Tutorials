...
class HMM_Module_M():
    def __init__(self, vbie, alphaInits, initParams=None, rate=0.9, m_str=25, update='svn'):
        self.vbie = vbie;
        self.m_str=m_str
        self.update = update
    ...
    def initGlobalParams(self):
        ...
        temp_beta1 = 15.0*alphaInits[1] if self.initParams is None else self.initParams['beta1']
    ...
    def m_step(self, y, A, localVars, update, tf=None):
        rho = (self.step+1)**(-self.rate)
        fH = lambda _X : np.conjugate(np.transpose(_X))
        K, N = A.shape
        [q_local_h_mu, q_local_h_sigma, q_local_l_alpha, q_local_l_beta, q_local_s] = localVars
        if 's' in update:
            # q(p01) = exp E_{q(s_n)} log p(p01) sum_n log[p(s|p01)]
            self.q_global_p01_alpha = (1-rho)*self.q_global_p01_alpha + rho*np.sum((1-q_local_s[:-1])*q_local_s[1:])
            self.q_global_p01_beta = (1-rho)*self.q_global_p01_beta + rho*np.sum((1-q_local_s[:-1])*(1-q_local_s[1:]))
            # q(p10) = exp E_{q(s_n)} log p(p10) sum_n log[p(s|p10)]
            self.q_global_p10_alpha = (1-rho)*self.q_global_p10_alpha + rho*np.sum(q_local_s[:-1]*(1-q_local_s[1:]))
            self.q_global_p10_beta = (1-rho)*self.q_global_p10_beta + rho*np.sum(q_local_s[:-1]*q_local_s[1:])
        if 'v' in update:
            # q(beta1) = exp E_{q(l_n)q(s_n)} [log p(beta1) sum_n logp(l_n|s_n; beta1)]
            self.q_global_beta1_alpha = (1-rho)*self.q_global_beta1_alpha + rho*np.sum(self.alpha1*q_local_s)
            self.q_global_beta1_beta = (1-rho)*self.q_global_beta1_beta \
                + rho*np.sum(q_local_s*(q_local_l_alpha/q_local_l_beta))
        if 'n' in update:
            # q(betaz) = exp E_{q(h)} log p(betaz) p(y|h; betaz)
            # if using real multivariate normal then q_local_l_F=0.5
            q_local_l_F = 1.0 
            self.q_global_betaz_alpha = (1-rho)*self.q_global_betaz_alpha + rho*q_local_l_F*K
            self.q_global_betaz_beta = (1-rho)*self.q_global_betaz_beta + rho*np.real(\
...
if __name__ == '__main__':
    ...
    for _i in range(20000):
        ...
        # prints
        print(cfp(params_em_50['p01']), cfp(params_em_50['p10']), cfp(params_em_50['beta1']/params_em_50['alpha1']), \
            params_em_50['alphaz']/cfp(params_em_50['betaz']), '','S:', 'A'+str(np.mean(dss)), s_em_50[-1], s_e[-1], \
                'N:', cfp(nmse_em_50[-1]), cfp(nmse_e[-1]), cfp(nmse_ga[-1]), sep='\t', end='', flush=True);
        print()
        if (_i+1)%50==0:
            print('True:', __p01, __p10, __VAR_B, __VAR_N, sep='\t')
            plt.close('all')
            ...
            plt.legend()
            plt.savefig('./convergence_nmse_500' if _i<=510 else './convergence_nmse')
